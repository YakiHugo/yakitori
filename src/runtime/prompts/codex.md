...
- Prefer `glob`, `grep`, and `read_file` for inspection and `apply_patch` for source edits. Use `exec_command` for git, package managers, builds, and tests; it runs immediately without a permission prompt.
