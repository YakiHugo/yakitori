...
Yakitori 是一个从零开始实现本地 coding-agent harness 和 GUI 的学习项目。产品方向是以具有持久记忆的 `Workmate` 为核心的 coding 工作台：Workmate 可以独立工作，也可以在共享任务 Room 中协作。
...
- 让每个 Workmate 在跨任务过程中保持持久身份、版本化 profile、受治理的记忆和可检查的历史。
- 允许多个 Workmate 并行处理同一个 Task，在同一个 Room 发布发现，并通过结构化 mention 请求别人关注。
- 区分共享 Message、每个接收者的 Delivery 和每个 Workmate 私有的 execution Session。
- 让 core 负责结构化执行、协作、工具、权限、持久化、恢复和回放。
- 构建一个 Task 工作台 GUI，展示共享讨论、Workmate 活动、terminal、diff、审批、artifact、worktree 和记忆来源。
...
+ 每个 Workmate Assignment 一条可检查的执行 lane
```

Room 并不是对 Slack 或 Raft 完整产品形态的复制，而是 coding 工作台内部的共享沟通边界。一条 Room Message 只保存一次；持久化 Delivery 决定哪些 Workmate 应该稍后补读、立即唤醒或在安全边界 steer。详细工具输出保留在各自的 execution Session 中，除非 Workmate 主动发布有界的发现或 artifact 引用。
...
- 不把 Workmate 当作永远运行的进程，也不把每条 transcript 消息静默提升为长期记忆。
...
Session kernel 现在被限定为单个 Workmate 的执行 lane。持久 Workmate 身份、Room/Task/Assignment 协作、durable Message delivery、model/tool runtime 和受治理的记忆已经是确定的架构方向，但尚未实现。
...
- [持久 Workmate 和 Room 协作](docs/decisions/0004-workmate-room-collaboration.md)
